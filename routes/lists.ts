...
export default function listRoutes(app: Application) {
    ...
    app.post('/api/loadFollowList', authenticateToken, uploadHandler(/\.(csv)$/).single('follows') , async(req: AuthorizedRequest, res: Response) => {
        if(req.file) {
            try {
                const petitionBy = await User.findByPk(req.jwtData?.userId)
                const lines: string[] = (await fs.readFile(req.file.path, 'utf8')).split('\n').map(elem => elem.split(',')[0]).slice(1);
                const okUsers: string[] = [];
                let localUsers = lines.filter(elem => elem.endsWith('@' +environment.instanceUrl)).map(elem => elem.split('@')[0].toLowerCase()).map(elem =>
                    User.findOne({
                        where: {
                            [Op.and]: [
                                sequelize.where(
                ...
                const remoteUsersPromises = lines.filter(elem => !elem.endsWith('@' +environment.instanceUrl)).map(elem => searchRemoteUser('@' +elem, petitionBy ));
                await Promise.allSettled(remoteUsersPromises.flat().concat(localUsers))
                localUsers = await Promise.allSettled(localUsers);
                const remoteUsers: {id: string, url: string}[] = (await Promise.allSettled(remoteUsersPromises.flat())).filter(elem => elem.status === 'fulfilled' && elem.value && elem.value.length > 0).map(elem => elem.status === 'fulfilled' ? elem.value : []).flat().filter(elem => elem != null).map(elem => {return {id: elem.id, url: elem.url}});
                const localUsersData: {id: string, url: string}[] = localUsers.filter(elem => elem.status === 'fulfilled').filter(elem => elem.value != undefined).map(elem => {return {id: elem.value.id, url: elem.value.url}})
                const foundUrls = remoteUsers.map(elem => elem.url).concat(localUsersData.map(elem => `@${elem.url}@${environment.instanceUrl}`))
